...
function initPullToRefresh() {
  window.addEventListener('touchstart', (e) => {
    ...
    if (window.scrollY === 0) {
      ...
      if (elements.loadingOverlay) {
        elements.loadingOverlay.style.transition = 'none';
        elements.loadingOverlay.classList.remove('hidden');
      }
    }
  }, { passive: true });
  ...
  window.addEventListener('touchmove', (e) => {
    ...
    pullDistance = Math.max(0, (touchCurrent - touchStart) * 0.4);

    if (pullDistance > 0 && window.scrollY === 0) {
      if (elements.loadingOverlay) {
        elements.loadingOverlay.style.transform = `translateY(${Math.min(pullDistance, PULL_THRESHOLD + 20)}px)`;

        // Add rotation based on pull
        if (elements.loadingSpinner) {
          const rotation = (pullDistance / PULL_THRESHOLD) * 360;
          elements.loadingSpinner.style.transform = `rotate(${rotation}deg)`;
        }
      }
    }
  }, { passive: true });
...
async function startPullRefresh() {
  if (isRefreshing) return;
  isRefreshing = true;

  if (elements.loadingOverlay) {
    elements.loadingOverlay.style.transition = 'all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
    elements.loadingOverlay.style.transform = `translateY(${PULL_THRESHOLD}px)`;
  }
  ...
  try {
    ...
    setTimeout(finishPullRefresh, 1000);
  }
}

function finishPullRefresh() {
  if (elements.loadingOverlay) {
    elements.loadingOverlay.style.transform = 'translateY(-100px)';
    setTimeout(() => {
      elements.loadingOverlay.classList.add('hidden');
      elements.loadingOverlay.style.transform = '';
      isRefreshing = false;
    }, 400);
  } else {
    isRefreshing = false;
...
function cancelPullRefresh() {
  if (elements.loadingOverlay) {
    elements.loadingOverlay.style.transition = 'all 0.3s ease';
    elements.loadingOverlay.style.transform = 'translateY(-100px)';
    setTimeout(() => {
      elements.loadingOverlay.classList.add('hidden');
      elements.loadingOverlay.style.transform = '';
