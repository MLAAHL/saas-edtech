const CACHE_NAME = 'teaching-v14';
const STATIC_CACHE = 'static-v14';
const API_CACHE = 'api-v14';
...
self.addEventListener('activate', (event) => {
    ...
    event.waitUntil(
        caches.keys().then(keys => {
            return Promise.all(
                keys
                    .filter(key => !key.includes('v14'))  // Delete ALL caches that are NOT v14
                    .map(key => {
...
self.addEventListener('fetch', (event) => {
    ...
    if (url.pathname.includes('/api/')) {
        // Skip SW cache if request has no-cache header (e.g., after adding a subject)
        ...
        // For other API calls or no-cache requests, network first with timeout
