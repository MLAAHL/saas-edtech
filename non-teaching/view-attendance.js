...
function showNotification(message, type = 'info') {
    ...
    border-radius: 12px;
    padding: 14px 18px;
    box-shadow: 0 8px 32px ${style.shadow}, 0 2px 8px rgba(0, 0, 0, 0.3);
    z-index: 99999;
    display: flex;
    align-items: center;
    gap: 12px;
  ...
  `;
    ...
    " onmouseover="this.style.background='rgba(255,255,255,0.1)'" onmouseout="this.style.background='rgba(255,255,255,0.06)'">
      <span class="material-symbols-rounded" style="font-size: 16px; color: #8A8A92;">close</span>
    </button>
  `;
...
function showConfirm(message, onConfirm, onCancel) {
  ...
  `;
    ...
    cancelBtn.addEventListener('mouseover', () => {
        cancelBtn.style.background = '#2A2A2D';
    });

    cancelBtn.addEventListener('mouseout', () => {
        cancelBtn.style.background = '#242427';
    });
...
function saveState() {
    ...
    localStorage.setItem('ntAttendanceViewState', JSON.stringify(state));
    console.log('💾 State saved:', state);
}

async function restoreState() {
    try {
        const savedState = localStorage.getItem('ntAttendanceViewState');
        ...
        if (state.viewMode === 'single' && state.date) {
            ...
            localStorage.removeItem('ntAttendanceViewState');
        }
...
function setupEventListeners() {
    ...
    if (editAttendanceBtn) editAttendanceBtn.addEventListener('click', enableEditMode);
...
async function loadRegister() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/attendance/register/${stream}/sem${semester}/${encodeURIComponent(subject)}`, { headers });
        const data = await response.json();
    ...
    try {
        ...
        <tr>
          <td colspan="10" style="text-align: center; padding: 60px;">
            <span class="material-symbols-rounded" style="font-size: 48px; color: var(--text-muted); display: block; margin-bottom: 16px;">event_busy</span>
            <p style="color: var(--text-muted); font-size: 14px;">No attendance records found for ${selectedDate}</p>
          </td>
        </tr>
      `;
...
function displayFullRegister() {
    ...
    registerData.sessions.forEach((session, index) => {
      ...
      <th class="session-header" data-session-id="${session._id}" data-session-index="${index}" style="text-align: center;">
        <div>
          ...
          <div style="font-size: 10px; margin-top: 4px; font-weight: 500; color: var(--text-muted);">${date}</div>
        </div>
      </th>
    `;
    });
    ...
    registerData.students.forEach((student, index) => {
      ...
      `;
        ...
        bodyHTML += `<td style="text-align: center; font-weight: 700; color: var(--green)">${student.presentCount}</td>`;
        bodyHTML += `<td style="text-align: center; font-weight: 700; color: var(--red)">${student.absentCount}</td>`;
        bodyHTML += `<td style="text-align: center; font-weight: 700; color: var(--purple)">${student.attendancePercentage}%</td>`;
        bodyHTML += '</tr>';
    });
...
async function deleteAttendance() {
    ...
    showConfirm(message, async () => {
        try {
          ...
          <tr>
            <td colspan="10" style="text-align: center; padding: 60px;">
              <span class="material-symbols-rounded" style="font-size: 48px; color: var(--green); display: block; margin-bottom: 16px; font-variation-settings: 'FILL' 1;">check_circle</span>
              <p style="color: var(--text-muted); font-size: 14px;">Records deleted successfully</p>
            </td>
          </tr>
        `;
    ...
    <tr>
      <td colspan="10" style="text-align: center; padding: 60px;">
        <div style="
          ...
          margin: 0 auto 16px;
        "></div>
        <p style="color: var(--text-muted); font-size: 14px;">Loading data...</p>
      </td>
    </tr>
  `;
